...
%
% (1) Loads data for the Euro Area or a country of interest. The input data are already de-seasonalized and
...
%  - sheet: 'EA','DE','FR','IT','ES','PT','NL','IE','EL','BE','AT' ---> default is 'EA' 
%  - spec.trans: 'light' = light transformations, 'heavy' = heavy transformations 
%    ---> default is 'light'
%  - whether to impute outliers and the Covid period ---> default is 'No',
%    IF 'Yes':
%        - method: imputation method for Covid period ---> default is 0 (impute missing values/ragged edges)
%         '': no imputation whatsoever
%          0: impute missing values at beginning of sample/ragged edges
%          1: impute outliers & missing values + Covid for real vars, 
%          2: standard imputation of outliers and missing values [MN16]
%          3: impute outliers & missing values + Kalman Smoother for Covid period, 
%          4: impute outliers & missing values + PCA for Covid period.
%  - q : n. of factors for imputation --> if q=99 (default), q is selected According to [BN02]
% ----------------------------------------------------------------------------------------------------------
% OUTPUT:
%		      Excel file (.xlsx) with transformed data, outliers and Covid
%		      period imputed.
% ----------------------------------------------------------------------------------------------------------
...
switch IDf
 case 'Q'                   																																																																																							      % quarterly aggregation
     [data,dates] = aggregate(data,dates,spec);																																																	                         
 case 'M'                 																																																																																							        % monthly subset
     data = data(:,strcmp(spec.Frequency,'M')); titles = titles(:,strcmp(spec.Frequency,'M'));
...
if isempty(method)
   IDi = ''; Xc = xt(sum(isnan(xt),2)==0,:);	dates = dates(sum(isnan(xt),2)==0,:);																																							% no imputation: remove all NaNs ---> balanced panel
end
...
if dsave==1
    ...
    if ~isempty(IDi); sIDi = join(['_',IDi],''); else; sIDi = IDi; end
    if strcmp(spec.trans,'light')
        writetimetable(TT,join([sheet,'data',IDf,sIDi,'_LT.xlsx'],''),'Sheet',sheet);
    elseif strcmp(spec.trans,'heavy')
...
switch answer
  ...
  case 'Yes'
        sheet = 'EA'; IDf = 'Q'; trans = 'light'; method = 0; q = 99;
        disp(join(['Country: ' sheet '; ' 'Frequency: ' IDf '; ' 'trans: ' trans '; ' ...
             'method: ', string(method) '; ' 'q: ' string(q) '.'],''))
  case 'No'
        sheet = inputdlg('Select country','Country',[1 20],{'EA'}); sheet = char(sheet);
        if isempty(sheet); disp('No country specified, setting to default (EA)'); sheet = 'EA'; end
        IDf = questdlg('Select frequency','Frequency','Q', 'M', 'Q'); IDf = char(IDf);
        trans = questdlg('Select transformation','Transformation','light','heavy','light');
        imp = questdlg('Impute outliers and Covid period?','Imputation','Yes','No','Yes'); imp = char(imp);
        if strcmp(imp,'Yes')
