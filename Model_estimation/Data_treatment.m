...
[data,dates] = aggregate(data,dates,spec);	                    % Aggretate to a quaterly level
                                               	% ------------------
                 
%% TRANSFORM DATA
spec.trans = trans;
xt = EAtransform(data,spec);

idNaN=(sum(isnan(xt),2)>0.7*size(xt,2));																																																																																	% remove rows with large numbers of NaNs
nld =(cumsum(idNaN)==(1:size(xt,1))'); 
xt(nld,:) = []; dates(nld,:) = [];																																																% --------------------------------------------------
...
if isempty(method)
   Xc = xt(sum(isnan(xt),2)==0,:);	dates = dates(sum(isnan(xt),2)==0,:);																																							% no imputation: remove all NaNs ---> balanced panel
end

if method==0 																																																					                                                       % Case 0: impute initial missing values/ragged edges up to t-1
 Xc = xt; Xc(1:end-1,:) = EMimputation(xt(1:end-1,:),q,opts); 																																																	% -------------------------------------
end                                                       
...
%------------------------------------------------------------																																																					       % IMPUTATION																																																																																																					      %==================================
    ...
    if method== 4
        ...
        for index=1:T21-T19																																																																																								          %================================== 
				        x = Xnan(1:T19+index-1,:); 																																																																																		    % select observations
			         mx = repmat(mean(x),size(x,1),1); sx = repmat(std(x),size(x,1),1); Xs = (x-mx)./sx;																										% standardize panel
				        pc = princfact(Xs,q); 																																																																																						 % PCA
				        Xnan(T19+index,:) = pc.chi(end,:).*sx(end,:) + mx(end,:);																																																								% impute observations at t+i
        end	
        disp("test")
...
%TT= array2timetable(Xc,'RowTimes',dates,'VariableNames',spec.Name);
...
for index = find(strcmp(spec.Frequency,'M'))'																																																																																% iterate over monthly variables
		
				idxNaN = isnan(data(:,index));																																																																																											% locate NaNs
				...
				if ~isempty(t0)																																																																																																						% min non-NaN date
								...
								if (quarter(tp)~=quarter(t0))||quarter(tp1)~=quarter(t0)																																																								 % if one value is missing, set as NaN 
											temp(dates==t0,index) = nan;																																																																																						% ---> need three full months to build quarterly value
								end																																																																																																														%-----------------------------------------------------
				end
				...
				if ~isempty(t1)																																																																																																						% max non-NaN date
								...
								if (quarter(tp)~=quarter(t1))||quarter(tp1)~=quarter(t1)																																																							  % if one value is missing, set as NaN 
											temp(dates==t1,index) = nan; 																																																																																					% ---> need three full months to build quarterly value
								end																																																																																																														%-----------------------------------------------------
				end
end
...
if spec.agg==1																																																																																																											% BALANCED PANEL
    %-----------------------------------------------------																																																															%-------------------------------
    Xt(:,spec.TR==0) = X(:,spec.TR==0);
    Xt(:,spec.TR==4) = X(:,spec.TR==4);%nothing

    Xt(:,spec.TR==1) = log(X(:,spec.TR==1));
    Xt(:,spec.TR==2) = log(X(:,spec.TR==2));%log

    Xt(2:end,spec.TR==3) = diff(log(X(:,spec.TR==3)));	% log-differences	

    Xt(2:end,spec.TR==5) = diff(X(:,spec.TR==5));	% differences																																																																			% second-differences
    %-----------------------------------------------------																																																														 %--------------------------------
elseif spec.agg==0																																																																																																							% MIXED-FREQUENCIES PANEL
    ...
    Xt(2:end,strcmp(spec.Frequency,'M')&spec.TR==3) = diff(log(X(:,strcmp(spec.Frequency,'M')&spec.TR==3)));											% log-differences: M vars
    Xt(:,strcmp(spec.Frequency,'Q')&spec.TR==3) = mdiff(log(X(:,strcmp(spec.Frequency,'Q')&spec.TR==3)),3,1);% log-differences: Q vars

    Xt(2:end,strcmp(spec.Frequency,'M')&spec.TR==5) = diff(X(:,strcmp(spec.Frequency,'M')&spec.TR==5));											% log-differences: M vars
    Xt(:,strcmp(spec.Frequency,'Q')&spec.TR==5) = mdiff(X(:,strcmp(spec.Frequency,'Q')&spec.TR==5),3,1);% differences: Q vars
...
%disp('Number of factors selected according to...');
...
function [X,out,n] = remove_outliers(X)
% =========================================================================
% Function to locate outliers and replace them with NaNs
%
%	An outlier is identified for each series whenever |abs(X-m_X)>iqr_X|,
% where:
%						- m_X = median of X
%						- iqr_X = interquartile range of X
% -------------------------------------------------------------------------
% SYNTAX: remove_outliers(X)
% where:
...
out = abs(X-repmat(mX,size(X,1),1)) > repmat(iqrX,size(X,1),1);																																																							 % identify outliers		% locate col/row of outliers
...
for index=kmax:-1:1
    ...
    Sigma(index)=mean(sum(ehat.*ehat/T));

    % Value of the information criterion when using i factors
    IC1(:,index)=log(Sigma(index))+CT(:,index);
...
for index=0:N:N*(p-1)                                                                                                        %=====================================
    res.AL(:,:,j) = res.A(index+1:index+N,:)';                                                                                   % lag i coefficients
    j = j+1;                                                                                                             % ------------------
end                                                                                                                      %=====================================                      
%---------------------------------------
...
switch method                                                                                                            %================== 
 case 1                                                                                                                  % Case 1: use lag 0
      %----------------------------------------------------                                                              %------------------ 
      if order==1; X = []; for index=0:p; X = cat(2,X,Y(p-index+1:end-index,:)); end                                                               
      else; X = []; for index=1:N; for j=0:p; X = cat(2,X,Y(p-j+1:end-j,index)); end; end
      end
       %----------------------------------------------------                                                              %------------------
  case 2                                                                                                                 % Case 2: no lag 0
      %----------------------------------------------------                                                              %------------------
      if order==1; X = []; for index=1:p; X = cat(2,X,Y(p-index+1:end-index,:)); end
...
for index=1:opts.pmax                                                                                                        %=======================================
    res = estVAR(Y(1:end,:),index,opts);                                                                                     % estimate VAR
    
    T = size(res.u,1); TT(:,index) = T;                                                                                      % effective sample size
    BIC(:,index) = -2*res.loglik/T + (log(T)/T)*N*res.k;                                                                     % Schwartz (Bayes) Information Criterion
    AIC(:,index) = -2*res.loglik/T + 2*N*res.k/T;                                                                            % Akaike Information Criteron
    HQC(:,index) = -2*res.loglik/T + 2*N*res.k*log(log(T))/T;                                                                % Hannan-Quinn Information Criterion
    LR(:,index) = log(det(res.Q_ML));                                                                                        % Likelihood-ratio 
end                                                                                                                      %=======================================
%--------------------------------------------------------
