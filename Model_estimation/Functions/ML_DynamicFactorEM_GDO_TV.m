% ML_DynamicFactorEM_GDO_TV - ML estimation of NSDFM with LT and TV deterministic components
...
%     GDO - if ==1 impose restrictions GDO:=GDP+GDI. GDP is variable ...
%                  ... number 1 while GDI is variable number 2
% 

% Matteo Luciani (matteoluciani@yahoo.it)

function [xitT,PtT,PtTm,xitt,xittm,Ptt,Pttm,A,Lambda,R,Q,b1,VECM,mu]=...
    ML_DynamicFactorEM_GDO_TV(Y,F00,P0s,A,Lambda,R,Q,s,q,d,p,mu,type,model,maxiter,tresh,cc,GDO,price)

try isnan(GDO); catch, GDO=1; end
try isnan(price); Jp=[36 38 40 43]; catch; price=0; end
...
GDOb=0;                                                                     % When we do not have Time Varaition in b ...
if GDO==1                                                                   % ... we need to impose b(GDP)=B(GDI) ...
    if isempty(isTV); GDOb=1; else; if isTV(1)~=pqs+1; GDOb=1; end          % ... with time variation this is taken 
    end                                                                     % ... care of by the Kalman filter
end                                                                         % -------------------------------------------

if GDO==1 || GDO==2;  GDOc=1; else; GDOc=0; end                             % Impose chi(GDP)=chi(GDO) even when D(cons) = D(GDO)

%%% =================== %%%
%%%  Start with E-Step  %%%
%%% =================== %%%
[xitt,xittm,Ptt,Pttm,loglik]=ML_KalmanFilter2(F00,P0s,Y,A,Lambda,R,Q,mu);   % Kalman Filter                           
...
for jj=1:maxiter         
    ...
    if GDOc==1; L(1:2,:)=repmat(mean(L(1:2,:)),2,1); end                    % GDO loadings
    ...
    if GDOb==1; b1(1:2)=mean(b1(1:2)); end                                  % slope for GDP=GDI 
    
    F00(isc3)=temp(1:sum(nJ3),2);                                           % starting value for WNwLT
    if GDOb==1; F00(isc3(1:2))=mean(F00(isc3(1:2))); end                    % starting value GDP=GDI
    ...
    %%% ======================================================= %%%
    if strcmp('VAR',model); % --------------------------------------------- % Estimate a VAR on Factors 
        ...
        VECM.alpha=[];  VECM.beta=[]; VECM.GL=[];
    elseif strcmp('VECM',model); % -------------------------------------------- % Estimate a VAR on Factors 
        [temp, alpha, beta,~,~, ut,Gamma,AL0]=BLL_VECM(F,p,d,10,1);
        VECM.alpha=alpha;  VECM.beta=beta; VECM.GL=Gamma;
        A(1:pq,1:pq)=ML_VAR_companion_matrix(AL0);  
        mu(1:q)=temp(1,:)';    
        Q(1:q,1:q) = cov(ut);          
    end
